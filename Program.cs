                ...
                {                    
                    ...
                    if (string.Equals(ourAnimals[i, 2], "Age: ", StringComparison.Ordinal) || string.Equals(ourAnimals[i, 2], "Age: ?", StringComparison.Ordinal))
                    {
                        while (true)
                        {
                            ...
                            {
                                // Enforce validation rule of converting the non-null value to numeric data type
                                if (int.TryParse(readResult, out int newAge) && newAge >= 0 && newAge <= 150)
                                {
                                    // Sets new age value
                    ...
                    // Checks if Physical description has any value different from default. If not, asks user for an entry
                    if (string.Equals(ourAnimals[i, 4], "Physical description: ", StringComparison.Ordinal))
                    {
                        while (true)
                        {
                            Console.WriteLine($"Enter the Physical description for {ourAnimals[i, 0]}");
                            readResult = Console.ReadLine();
                            if (!string.IsNullOrWhiteSpace(readResult) && readResult.Length > 0)
                            {
